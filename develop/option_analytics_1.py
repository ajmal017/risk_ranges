...
# http://www.smileofthales.com/computation/options-greeks-python/
...
class BSMerton:
    ...
    def vega(self):
        vega1 = 0.01 * self.S * e ** (-self.q * self.T) * norm.pdf(self.d1) * (self.T ** 0.5)
        return vega1
    ...
    def dDeltadTime(self):
        dfq = e ** (-self.q * self.T)
        if self.Type == 1:
            return (1.0 / 365.0) * -dfq * (norm.pdf(self.d1) * ((self.r-self.q) / (self.sigmaT) - self.d2 / (2 * self.T)) + (-self.q) * norm.cdf(self.d1))
        else:
            return (1.0 / 365.0) * -dfq * (norm.pdf(self.d1) * ((self.r-self.q) / (self.sigmaT) - self.d2 / (2 * self.T)) + self.q * norm.cdf(-self.d1))
